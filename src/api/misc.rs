...
/// Models advertised by `/v1/models` and `/code/v1/models`.
///
/// Only the latest version of each family is listed so the proxy matches what
/// the official Claude desktop app exposes by default. The `/v1/messages`
/// endpoint forwards `model` to Anthropic without consulting this list, so
/// clients that already know an older versioned model name (e.g.
/// `claude-sonnet-4-5-20250929`) can still send it directly — only the
/// discovery list is trimmed.
///
/// Variants:
///  * base                — default behavior
///  * `-thinking`         — extended-thinking mode (ClewdR translates to the
///                          upstream `thinking` field)
///  * `-1M`               — 1M-context window (Anthropic gates this on
///                          accounts without long-context credits; expect
///                          HTTP 429 if the account isn't entitled)
///  * `-1M-thinking`      — both of the above
///
/// Haiku is intentionally absent: Anthropic's Claude Code subscription path
/// (which ClewdR's OAuth flow uses) does not currently expose Haiku.
